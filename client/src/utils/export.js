  ...
  try {
    ...
    canvas.elements.forEach(element => {
      ...
      switch (element.type) {
        ...
        case 'rhombus':
          // Draw rhombus as a polygon
          ctx.save();
          ctx.strokeStyle = element.stroke;
          ctx.fillStyle = element.fill === 'transparent' ? 'transparent' : element.fill;
          ctx.lineWidth = element.strokeWidth;
          ctx.globalAlpha = element.opacity / 100;
          
          // Create points for the rhombus (diamond)
          const points = [
            { x: x + element.width / 2, y: y }, // Top
            { x: x + element.width, y: y + element.height / 2 }, // Right
            { x: x + element.width / 2, y: y + element.height }, // Bottom
            { x: x, y: y + element.height / 2 }, // Left
          ];
        ...
        case 'image':
          ...
          if (element.src) {
            ...
            // Only draw if image is already loaded
    ...
    canvas.elements.forEach(element => {
      ...
      switch (element.type) {
        ...
        case 'rhombus':
          ...
          node.setAttribute('stroke', element.stroke);
          node.setAttribute('stroke-width', element.strokeWidth);
          node.setAttribute('fill', element.fill === 'transparent' ? 'none' : element.fill);
          break;
