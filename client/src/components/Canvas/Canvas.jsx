...
const Canvas = () => {
  ...
  }), [zoom, scrollX, scrollY, currentCanvas, selectedElementIds, setSelectedElements, setCurrentTool, isCanvasLocked,
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // Pass keyboard events to tool (only when not typing and canvas is not locked)
      if (!isTyping && currentTool.current && currentTool.current.onKeyDown && !isCanvasLocked) {
  ...
  const handleMouseDown = useCallback(
    (e) => {
      // If canvas is locked, don't handle any mouse events except panning
      if (isCanvasLocked) {
        if (isSpacePressed.current || e.button === 1) {
          // Middle mouse button or space+click for panning is still allowed
  ...
  const handleMouseMove = useCallback(
    (e) => {
      // If canvas is locked, only allow panning
      if (isCanvasLocked && !isPanning.current) {
        return;
      }
    ...
    [scrollX, scrollY, setScroll, getToolState, isCanvasLocked]
  );

  // Handle mouse up
  const handleMouseUp = useCallback((e) => {
    // If canvas is locked, only allow panning to be released
    if (isCanvasLocked && !isPanning.current) {
      return;
    }
  ...
  }, [zoom, scrollX, scrollY, currentCanvas, selectedElementIds]); // Removed fillColor from dependencies
  ...
  }, [dimensions, scrollX, scrollY, zoom, currentCanvas, getToolState]); // Removed fillColor from dependencies
