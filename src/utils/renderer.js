...
/**
 * Render a rhombus element
 * @param {RoughCanvas} rc - RoughCanvas instance
 * @param {Object} element - Rhombus element to render
 */
export const renderRhombus = (rc, element) => {
  const options = getRoughOptions(element);
  
  // Create points for the rhombus (diamond)
  const points = [
 ...
 * @param {CanvasRenderingContext2D} ctx - Canvas 2D context
 * @param {Object} element - Image element to render
 * @param {Number} zoom - Current zoom level
 */
export const renderImage = (ctx, element, zoom = 1) => {
  if (!element.src) return;
  
  const img = new Image();
  img.src = element.src;
  
  // Only render if image is already loaded
  if (img.complete && img.naturalWidth !== 0) {
    ...
    // When image loads, trigger a re-render
    img.onload = () => {
      // This would normally trigger a canvas re-render
      // In a real implementation, we would call a function to re-render the canvas
    };
  }
};
